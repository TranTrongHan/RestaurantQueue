...
public class UserService {
    ...
    public UserResponse createUser(UserCreationRequest request, MultipartFile file) throws IOException {
        ...
        // UserCreationRequest request1 =
        // UserCreationRequest.builder().username(request.getUsername()).build();
        User user = userMapper.toUser(request);
        user.setPassword(passwordEncoder.encode(request.getPassword()));
        user.setRole(User.Role.CUSTOMER);
        user.setImage(imageUrl);
        user.setAuthProvider(User.AuthProvider.LOCAL);
        // UC01: Gán hạng New Member khi đăng ký
        membershipTierRepository.findByTierName("New Member").ifPresent(user::setMembershipTier);
        user.setTotalSpending(java.math.BigDecimal.ZERO);
    ...
    public UserResponse findUser(String userID) {
        return userMapper.toUserResponse(
                this.userRepository.findById(userID).orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED)));
    }

    public UserResponse updateUser(String userID, UserUpdateRequest request) throws IOException {
        User persisted = this.userRepository.findById(userID)
